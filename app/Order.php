...
class Order extends Model
{
    ...
    public function getExpeditionType(){
        ...
        $delivery="";
        if(config('app.isft')){
            $delivery=$this->delivery_method==1?__('HOME DELIVERY'):__('Pickup');
        }
        //QR or WP
        if(config('app.isqrsaas')){
            if(config('settings.is_whatsapp_ordering_mode')){
                ...
                //POS
                $delivery=$this->delivery_method==1?__('Delivery'):($this->delivery_method==3?__('IN-STORE PICKUP'):__('HOME DELIVERY'));
            }else{
                //QR
    ...
    {
        return $this->belongsToMany(\App\Items::class, 'order_has_items', 'order_id', 'item_id')->withPivot(['qty', 'extras', 'vat', 'vatvalue', 'variant_price', 'variant_name','id','kds_finished', 'comment'])->withTrashed();
    }
    ...
    public function restaurantCallbacks()
    {
        return $this->hasMany(\App\Models\RestaurantCallback::class, 'order_id');
    }
}
